...
void SetSDAOut()
{
   __raw_writel((__raw_readl(S3C64XX_GPBCON)&(0x1ffffff)),S3C64XX_GPBCON);
	//__raw_writel((readl(S3C2410_GPEUP)|(1<<15)),S3C2410_GPEUP);
}

void SetSDAInput()
{
	__raw_writel((__raw_readl(S3C64XX_GPBCON)&(0x0ffffff)),S3C64XX_GPBCON);
}

void SetSCLOut()
{
	__raw_writel((__raw_readl(S3C64XX_GPBCON)&(0xf1fffff)),S3C64XX_GPBCON);
}

void SetSCLInput()
{
	__raw_writel((__raw_readl(S3C64XX_GPBCON)&(0xf0ffffff)),S3C64XX_GPBCON);
}
	...
	case I2C_WRITE_DATA:
		{
			...
			for(i=0;i<imsg.len;i++)
			{
				if(writeOnedate(imsg.addr+i,imsg.buffer[i])==0)
				{
					break;
				}
			}
