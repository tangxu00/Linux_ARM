...
struct s3c6410_button_platform
{
  struct button_irq_desc *button_irq;
  int button_num;
};
struct s3c6410_button_platform *button_platform;
static volatile int ev_press=0;
static int button_major=0;

struct cdev *button_cdev=NULL;
static volatile unsigned char key_val[]={'0','0','0','0','0','0','0','0'};
...
static irqreturn_t button_interrupt(int irq,void *dev_id)
{
  ...
  if(down!=(key_val[number]&1)){
    key_val[number]='0'+down;
    ev_press =1;
...
int button_read(struct file *filp,char __user *buf,size_t count,loff_t *pos)
{
  unsigned long err=0;
  if(!ev_press)
  {
  ...
  ev_press=0;
  err=copy_to_user((void*)buf,(const void*)(&key_val),min(sizeof(key_val),count));
  return err ? -EFAULT : min(sizeof(key_val),count);
}
...
int button_probe(struct platform_device *pdev)
{
  ...
  int err;
  dev_t dev;
  button_platform=(struct s3c6410_button_platform*)pdev->dev.platform_data;
  button_irqs=button_platform->button_irq;
  printk("driver match ok!\n");
  printk("%s,%x,%d\n",button_irqs[0].name,button_irqs[0].irq,button_irqs[0].num);
  if(button_major)
  {
...
struct platform_driver s3c6410_button_drv={
  .probe=button_probe,
...
{
  return platform_driver_register(&s3c6410_button_drv);
}
static  void button_exit(void)
{
  platform_driver_unregister(&s3c6410_button_drv);
}
